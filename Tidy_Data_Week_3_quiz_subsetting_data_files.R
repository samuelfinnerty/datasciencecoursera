...
FGDP <- data.table::fread('https://d396qusza40orc.cloudfront.net/getdata%2Fdata%2FGDP.csv'
                          ...
                          , nrows = 190
                          , select = c(1, 2, 4, 5)
                          , col.names=c("CountryCode", "Rank", "Economy", 
                                        "Total")
)

# Download data and read FGDP data into data.table
FEDSTATS_Country <- 
  data.table::fread('https://d396qusza40orc.cloudfront.net/getdata%2Fdata%2FEDSTATS_Country.csv'
)
...
mergedDT[`Income Group` == "Lower middle income", .N, by = c("Income Group", 
                                                             "quantileGDP")]
